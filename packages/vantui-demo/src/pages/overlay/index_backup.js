...
class _C extends React.Component {
  render() {
    const { show, showEmbedded } = this.data
    return (
      <Block>
        <DemoBlock title="基础用法" padding>
          <VanButton type="primary" onClick={this.onClickShow}>
            显示遮罩层
          </VanButton>
          <VanOverlay show={show} onClick={this.onClickHide}></VanOverlay>
        </DemoBlock>
        <DemoBlock title="嵌入内容" padding>
          <VanButton type="primary" onClick={this.onClickShowEmbedded}>
            嵌入内容
          </VanButton>
          <VanOverlay show={showEmbedded} onClick={this.onClickHideEmbedded}>
